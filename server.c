...
//room
#define SIGNAL_CREATEROOM "SIGNAL_CREATEROOM"
#define SIGNAL_ENTERROOM "SIGNAL_ENTERROOM"
...
struct roomLinkList
{
  ...
  char user1;
  char user2;
  struct roomLinkList *next;
};
typedef struct roomLinkList *nodeRoom;

nodeRoom CreateNode(char* hostname){
    nodeRoom tmp; // declare a node
    tmp = (nodeRoom)malloc(sizeof(struct roomLinkList)); // Cấp phát vùng nhớ dùng malloc()
    tmp->next = NULL;// Cho next trỏ tới NULL
    tmp->user1 = hostname; // Gán giá trị cho Node
    tmp->user2 = NULL;
    tmp->roomId = roomId++;
...
nodeRoom AddTail(nodeRoom head, char* hostname){
    nodeRoom temp, p;// Khai báo 2 node tạm temp và p
    temp = CreateNode(hostname);//Gọi hàm createNode để khởi tạo node temp có next trỏ tới NULL và giá trị là value
    if(head == NULL){
...
nodeRoom DelAt(nodeRoom head, int position){
...
int Search(nodeRoom head, int id){
    int position = 0;
    for(nodeRoom p = head; p != NULL; p = p->next){
        if(p->roomId == id){
            return position;
        }
        ++position;
...
int handleDataFromClient(int fd){
  ...
  else if (strcmp(str, SIGNAL_CREATEROOM) == 0)
  {
    //CreateNode()
    printf("Your room id: %d\n", roomId++);

  }
  else if (strcmp(str, SIGNAL_ENTERROOM) == 0)
  {
    /* code */
  }
